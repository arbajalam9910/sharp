...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        ...
        msg_text = f"*User {target_user_id} removed. Clearance by {@TRUST_TRUST9}*"
...
def send_welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        f"👊 *Welcome to Command, Agent. Choose your directive.* Managed by {@TRUST_TRUST9}",
        reply_markup=markup,
        parse_mode='Markdown'
    )
...
def handle_message(message):
    if message.text == "💀 Initiate Attack 🔥":
        bot.reply_to(message, f"*Command received. Preparing deployment. Stand by, {@TRUST_TRUST9}*", parse_mode='Markdown')
